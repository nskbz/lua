...
func parseWhileStat(l *lexer.Lexer) ast.Stat {
	...
	l.AssertToken(lexer.TOKEN_KW_DO)
...
func parseIfStat(l *lexer.Lexer) ast.Stat {
	...
	for {
		nt := l.LookToken()
		if nt.Kind() != lexer.TOKEN_KW_ELSEIF {
			break
		}
		l.NextToken()
		exps = append(exps, parseExp(l))
	...
	nt := l.LookToken()
	if nt.Kind() == lexer.TOKEN_KW_ELSE {
		l.NextToken()
...
func parseForStat(l *lexer.Lexer) ast.Stat {
	...
	name := l.AssertIdentifier()
	names = append(names, name.Val())
	l.NextToken()
	nt := l.LookToken()
	if nt.Kind() == lexer.TOKEN_OP_ASSIGN {
		_parseForNumStat(l, f.Line(), names, exps)
	} else if nt.Kind() == lexer.TOKEN_SEP_COMMA {
		_parseForInStat(l, names, exps)
	}
